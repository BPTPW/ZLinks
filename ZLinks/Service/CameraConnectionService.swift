...
final class CameraConnectionService: ObservableObject {
    ...
    private func runLiveViewLoop(generation: Int) async {
        ...
        {
            do {
                if let frame = try await fetchLiveViewFrame(on: channel) {
                    liveViewImage = frame
                    liveViewError = nil
            ...
            // 让出命令通道即可。固定 33ms 的等待会白白压低刷新率，
            // 相机侧返回 busy 时 fetchLiveViewFrame 会自行退避。
            await Task.yield()
        }
    ...
    private func fetchLiveViewFrame(on connection: any PTPChannel) async throws -> UIImage? {
...
// PTP/IP 报文与通道实现已迁移到 CameraLink.swift，USB 链路见 USBCameraLink.swift。
